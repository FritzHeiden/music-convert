...
convert_flac() {
  ...
  #outfile=$(echo "$source_file" | sed "s/flac$/opus/" | sed -E "s|$basename||g")
  outfile=$(echo "$source_file" | sed "s/flac$/opus/")
  ...
  mkdir -p "$outdir"
  echo "$outfile"
  ffmpeg -loglevel error -y -i "$source_file" -c:a libopus -b:a 192K -vn -map_metadata 0 "$outfile"
}
