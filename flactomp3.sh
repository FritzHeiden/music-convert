...
convert_flac() {
  ...
  outfile=$(echo "$source_file" | sed "s/flac$/mp3/")
  #outfile=$(echo "$source_file" | sed "s/flac$/mp3/" | sed -E "s|$basename||g")
  ...
  mkdir -p "$outdir"
  echo "$outfile"
  ffmpeg -loglevel error -y -i "$source_file" -ab 320k -map_metadata 0 -id3v2_version 3 "$outfile"
}
